...
public final class AirTripsGenerationTesting implements ModelComponent {
    ...
    private ConcurrentMap<Integer, Map<Integer, Double>> airSkimTravelTimeSec = new ConcurrentHashMap<>();
    private ConcurrentMap<Integer, Map<Integer, Double>> airSkimDistanceM = new ConcurrentHashMap<>();
    private ConcurrentMap<Integer, Map<String, Double>> airHubSkimTravelTimeSec = new ConcurrentHashMap<>();
    private ConcurrentMap<Integer, Map<String, Double>> airHubSkimDistanceM = new ConcurrentHashMap<>();


    public AirTripsGenerationTesting() {
    }
...
//        obtainAirRoutesForTrips(dataSet);
        generateAirSkims(dataSet);
        try {
            writeAirSkims(airSkimTravelTimeSec, airSkimDistanceM);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }

        generateAirSkimsToHubs(dataSet);
        try {
            writeAirToHubsSkims(airHubSkimTravelTimeSec, airHubSkimDistanceM);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
    ...
    private void generateAirSkimsToHubs(DataSet dataSet) {
        Collection<Zone> zones = dataSet.getZones().values();
        logger.info("Generate air skims for" + dataSet.getZones().size() + " zones");
        AtomicInteger counter = new AtomicInteger(0);
        String[] airportHubNames = {"LHR", "STN", "LGW", "LCY", "LTN", "SEN",
                "CDG", "ORY",
                "FCO", "CIA",
                "MAD",
                "AMS",
                ...
                "SIN",
                "TPE"};
        int[] airportHubIds = {218, 219, 220, 221, 222, 223,
                102, 103,
                146, 151,
                87,
                161,
        ...
        zones.parallelStream().forEach(zone -> obtainAirRoute(dataSet, zone, airportHubNames, airportHubIds));
        logger.info("Finished Assigning Air Route Model");
    }

    private void obtainAirRoute(DataSet dataSet, Zone origZone, String[] airportHubNames, int[] airportHubIds) {
        ...
        ZoneGermany originZone = (ZoneGermany) origZone;
        int originZoneId = originZone.getId();
        if (originZone.getZoneType().equals(ZoneTypeGermany.EXTOVERSEAS)) {
            if (originZoneId == 11876) {
                originAirport = dataSet.getAirportFromId(225);
            } else if (originZoneId == 11877) {
                originAirport = dataSet.getAirportFromId(274);
            } else if (originZoneId == 11878) {
                originAirport = dataSet.getAirportFromId(296);
            } else {
                originAirport = dataSet.getAirportFromId(299);
            }
        }

        airHubSkimTravelTimeSec.putIfAbsent(originZoneId, new HashMap<>());
        airHubSkimDistanceM.putIfAbsent(originZoneId, new HashMap<>());

        for (int seq = 0; seq < airportHubIds.length; seq++) {
            System.out.println("Calculate origin: " + originZoneId + " to airport hub: " + airportHubNames[seq]);
            if (originZone.getClosestAirportId() == airportHubIds[seq]) {
                ...
                destinationAirport = dataSet.getAirportFromId(airportHubIds[seq]);
                ...
                if (checkIfDirectFlightExists(originAirport, destinationAirport)) {
                    ...
                    if (connectedAirports.get(originAirport).get(destinationAirport).get("stops") == 0) {
                        //direct flight
                        airRoute = assignRoute(dataSet, originAirport, destinationAirport, originZoneId);
                    } else {
                        ...
                        float travelTimeBetweenAirports = getTotalTravelTime(dataSet, originZoneId, originAirport, destinationAirport);
                        float travelTimeFromHubOrigin = Float.MAX_VALUE;

                        if (checkIfDirectFlightExists(originHub, destinationAirport)) {
                            travelTimeFromHubOrigin = getTotalTravelTime(dataSet, originZoneId, originHub, destinationAirport);
                        }

                        float minTravelTime = Math.min(travelTimeBetweenAirports, travelTimeFromHubOrigin);
                        ...
                        if (minTravelTime == travelTimeBetweenAirports) {
                            ...
                            airRoute = assignRoute(dataSet, originHub, destinationAirport, originZoneId);
                        }
                    }
                    ...
                    if (checkIfDirectFlightExists(originAirport, destinationAirport)) {
                        travelTimeBetweenAirportAndMain = getTotalTravelTime(dataSet, originZoneId, originAirport, destinationAirport);
                    }
                    if (checkIfDirectFlightExists(originMain, destinationAirport)) {
                        travelTimeBetweenMainAirports = getTotalTravelTime(dataSet, originZoneId, originMain, destinationAirport);
                    }
                    if (checkIfDirectFlightExists(originHub, destinationAirport)) {
                        travelTimeFromHubOrigin = getTotalTravelTime(dataSet, originZoneId, originHub, destinationAirport);
                    }

                    float minTravelTime = Math.min(Math.min(travelTimeFromHubOrigin, travelTimeBetweenMainAirports), travelTimeBetweenAirportAndMain);


                    if (minTravelTime < Float.MAX_VALUE) {
                        if (minTravelTime == travelTimeFromHubOrigin) {
                            airRoute = assignRoute(dataSet, originHub, destinationAirport, originZoneId);
                        } else if (minTravelTime == travelTimeBetweenAirportAndMain) {
                            airRoute = assignRoute(dataSet, originAirport, destinationAirport, originZoneId);
                        } else {
                            airRoute = assignRoute(dataSet, originMain, destinationAirport, originZoneId);
                        }
                    } else {
                        airRoute = assignIntrazonalTrip();
                    }
            ...
            airHubSkimTravelTimeSec.get(originZoneId).put(airportHubNames[seq], Double.valueOf(airRoute.get("totalTime_sec")));
            airHubSkimDistanceM.get(originZoneId).put(airportHubNames[seq], Double.valueOf(airRoute.get("totalDistance_m")));
        }


    }

    private static void writeAirToHubsSkims(Map<Integer, Map<String, Double>> airHubSkimTravelTimeSec, Map<Integer, Map<String, Double>> airHubSkimDistanceM) throws FileNotFoundException {

        PrintWriter pwTime = new PrintWriter("C:/models/germanymodel/skims/airToHubsTravelTimeSec_europe.csv");
        pwTime.print("FROM;TO;VALUE");
        pwTime.println();
        for (int fromZone : airHubSkimTravelTimeSec.keySet()) {
            for (String toAirport : airHubSkimTravelTimeSec.get(fromZone).keySet()) {
                StringBuilder string = new StringBuilder();
                string.append(fromZone);
                string.append(";");
                string.append(toAirport);
                string.append(";");
                string.append(airHubSkimTravelTimeSec.get(fromZone).get(toAirport));
                pwTime.print(string);
                pwTime.println();
            }
        }
        ...
        PrintWriter pwDistance = new PrintWriter("C:/models/germanymodel/skims/airToHubsDistanceM_europe.csv");
        pwDistance.print("FROM;TO;VALUE");
        pwDistance.println();
        for (int fromZone : airHubSkimTravelTimeSec.keySet()) {
            for (String toZone : airHubSkimTravelTimeSec.get(fromZone).keySet()) {
                StringBuilder string = new StringBuilder();
                string.append(fromZone);
                string.append(";");
                string.append(toZone);
                string.append(";");
                string.append(airHubSkimTravelTimeSec.get(fromZone).get(toZone));
                pwDistance.print(string);
                pwDistance.println();
            }
        }
        pwDistance.close();

        System.out.println("Check air to hubs skims");
    }


    private void generateAirSkims(DataSet dataSet) {
        Collection<Zone> zones = dataSet.getZones().values();
        logger.info("Generate air skims for" + dataSet.getZones().size() + " zones");
        AtomicInteger counter = new AtomicInteger(0);
    ...
    private void obtainAirRoute(DataSet dataSet, Zone origZone) throws FileNotFoundException {

        Map<String, Float> airRoute = new HashMap<>();

        ZoneGermany originZone = (ZoneGermany) origZone;
        int originZoneId = originZone.getId();
        airSkimTravelTimeSec.putIfAbsent(originZoneId, new HashMap<>());
        airSkimDistanceM.putIfAbsent(originZoneId, new HashMap<>());
        for (Iterator<Zone> zoneIterator = dataSet.getZones().values().iterator(); zoneIterator.hasNext(); ) {
            ZoneGermany destinationZone = (ZoneGermany) zoneIterator.next();
            int destinationZoneId = destinationZone.getId();

            Airport originAirport;
            Airport destinationAirport;
            System.out.println("Calculate origin: " + originZoneId + " to destination: " + destinationZoneId);
            ...
            if (destinationZone.getZoneType().equals(ZoneTypeGermany.EXTOVERSEAS)) {
...
//            if (origZone.getZoneType().equals(ZoneTypeGermany.EXTOVERSEAS)) {
...
//            }
            if (originZone.getId() == destinationZone.getId() || originZone.getClosestAirportId() == destinationZone.getClosestAirportId()) {
                //same zone - no flights allowed;
                //same airport - no flights allowed
                airRoute = assignIntrazonalTrip();
            } else {
            ...
            airSkimTravelTimeSec.get(originZoneId).put(destinationZoneId, Double.valueOf(airRoute.get("totalTime_sec")));
            airSkimDistanceM.get(originZoneId).put(destinationZoneId, Double.valueOf(airRoute.get("totalDistance_m")));
        }

    }

    private static void writeAirSkims(Map<Integer, Map<Integer, Double>> airSkimTravelTimeSec, Map<Integer, Map<Integer, Double>> airSkimDistanceM) throws FileNotFoundException {

        PrintWriter pwTime = new PrintWriter("C:/models/germanymodel/skims/airTravelTimeSec_europe.csv");
        pwTime.print("FROM;TO;VALUE");
        pwTime.println();
        for (int fromZone : airSkimTravelTimeSec.keySet()) {
            for (int toZone : airSkimTravelTimeSec.get(fromZone).keySet()) {
                StringBuilder string = new StringBuilder();
                string.append(fromZone);
                string.append(";");
                string.append(toZone);
                string.append(";");
                string.append(airSkimTravelTimeSec.get(fromZone).get(toZone));
                pwTime.print(string);
                pwTime.println();
            }
        }
        ...
        PrintWriter pwDistance = new PrintWriter("C:/models/germanymodel/skims/airDistanceM_europe.csv");
        pwDistance.print("FROM;TO;VALUE");
        pwDistance.println();
        for (int fromZone : airSkimTravelTimeSec.keySet()) {
            for (int toZone : airSkimTravelTimeSec.get(fromZone).keySet()) {
                ...
                string.append(toZone);
                string.append(";");
                string.append(airSkimDistanceM.get(fromZone).get(toZone));
                pwDistance.print(string);
                pwDistance.println();
            }
        }
        ...
        System.out.println("Check air skims");
    }
    ...
    private Map<String, Float> assignIntrazonalTrip() {
        ...
        airRouteAttributes.put("totalTime_sec", Float.POSITIVE_INFINITY);
        airRouteAttributes.put("totalDistance_m", Float.POSITIVE_INFINITY);
        return airRouteAttributes;

    }

    private float getTotalTravelTime(DataSet dataSet, int originZoneId, Airport originAirport, Airport destinationAirport) {
    ...
    private Map<String, Float> assignRoute(DataSet dataSet, Airport originAirport, Airport destinationAirport, int originZoneId) {
...
//            }
    ...
    private void calculateMainAirportAndHubForZone(DataSet dataSet) {
        ...
        for (Airport airport : mainAirports) {
            ...
            if (airport.getZone().getZoneType().equals(ZoneTypeGermany.GERMANY)||airport.getZone().getZoneType().equals(ZoneTypeGermany.EXTEU)) {
