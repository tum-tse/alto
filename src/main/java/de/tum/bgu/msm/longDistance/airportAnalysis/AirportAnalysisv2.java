...
public final class AirportAnalysisv2 implements ModelComponent {

    private static final Logger logger = Logger.getLogger(AirportAnalysisv2.class);
        ...
        for (Airport origin : dataSet.getGermanAirports()){
            for (Airport destination : airports) {
                if (!checkIfDirectFlightExists(origin, destination)){
                    ...
                    origin = routeIfFlightRoutedFromOriginHubOrDestinationHub(origin, destination, originHub, destinationHub);
                    airports.add(origin);
                }
            }
        }
    ...
    private Airport routeIfFlightRoutedFromOriginHubOrDestinationHub(Airport origin, Airport destination, Airport originHub, Airport destinationHub) {
        ...
        for (Airport airport : dataSet.getGermanAirports()){
            ...
            for (Flight flight : airport.getFlights()) {
                ...
                for (AirLeg leg : flight.getLegs()) {
                    ...
                    pw.print(leg.getId());
                    pw.print(",");
                    pw.print(leg.getOrigin());
                    pw.print(",");
                    pw.print(leg.getDestination());
                    pw.print(",");
                    pw.print(leg.getCost());
                    pw.print(",");
                    pw.println(leg.getFrequency());
