...
public class SyntheticPopulationReaderOntario implements SyntheticPopulationReader {

    private static Logger logger = Logger.getLogger(SyntheticPopulationReaderOntario.class);
    ...
    public SyntheticPopulationReaderOntario() {
    }
    ...
    public void populateZones() {
        for (Household hh : dataSet.getHouseholds().values()) {
            ZoneOntario zone = ((HouseholdOntario) hh).getZone();
            zone.addHouseholds(1);
            zone.addPopulation(((HouseholdOntario) hh).getHhSize());
        }
...
//                int numKids = Integer.parseInt(lineElements[posKids]);
    ...
    private Map<Integer, Person>  readSyntheticPersons(Map<Integer, Household> householdMap) {
        ...
        try (BufferedReader in = new BufferedReader(new FileReader(ppFilename))) {
            ...
            while ((recString = in.readLine()) != null) {
                ...
                HouseholdOntario hh = (HouseholdOntario) householdMap.get(hhId);

                if (hh != null) {
                    Person pp = new PersonOntario(id, hhId, age, gender, occupation, education, workStatus, hh);  // this automatically puts it in id->household map in Household class
                    personMap.put(id, pp);
                } else {
                    if (logUnmatchedHh) {
    ...
    public void writeSyntheticPopulation() {
        ...
        for (Person person : dataSet.getPersons().values()) {
            PersonOntario trav = (PersonOntario) person;
